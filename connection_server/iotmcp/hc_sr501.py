...
mcp = FastMCP(
    "HC-SR501",
    description="HC-SR501 PIR motion sensor PC data monitoring via MCP. Supports motion detection reporting."
)
...
async def read_motion(
    ...
    """
    Read motion detected state from the HC-SR501 sensor.

    Parameters:
    - Real_time: if True, collect all valid data within 10 seconds and return as list.

    Returns:
    A motion state report, or list of reports if Real_time is True.
    """
    cmd = {"sensor": "HC-SR501", "command": "read_motion"}
    ...
    if Real_time:
        ...
        return {"status": "success", "data": collected, "count": len(collected) if isinstance(collected, list) else 0}
    else:
...
async def read_all(
    ...
    """
    Read full report from the HC-SR501 sensor.

    Parameters:
    - Real_time: if True, collect all valid data within 10 seconds and return as list.

    Returns:
    A full motion report with timestamp, or list of such reports if Real_time is True.
    """
    cmd = {"sensor": "HC-SR501", "command": "read_all"}
    response = sess.post("http://localhost:8000/send_cmd", json=cmd)
    if response.status_code != 200:
        return {"status": f"Web Error {response.status_code}"}
    data = response.json()
