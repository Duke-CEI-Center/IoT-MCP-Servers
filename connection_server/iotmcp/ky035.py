...
mcp = FastMCP(
    "KY035",
    description="KY035 PC data monitoring via MCP. Supports raw ADC reading, voltage, field strength, and switch state."
)
...
async def read_raw(
    ...
    """
    Read raw ADC values from the KY035 sensor.
    ...
    Returns:
    A raw ADC reading with time stamp, or list of readings if Real_time is True.
    """
    cmd = {"sensor": "KY035", "command": "read_raw", "interval": interval, "duration": duration}
...
async def read_voltage(
    ...
    """
    Read voltage values from the KY035 sensor.
    ...
    Returns:
    A voltage reading with time stamp, or list of readings if Real_time is True.
    """
    cmd = {"sensor": "KY035", "command": "read_voltage", "interval": interval, "duration": duration}
    response = sess.post("http://localhost:8000/send_cmd", json=cmd)
    id = None
    if response.status_code == 200:
        data = response.json()
...
async def read_field(
    ...
    """
    Read magnetic field strength from the KY035 sensor.
    ...
    Returns:
    A field strength reading with time stamp, or list of readings if Real_time is True.
    """
    cmd = {"sensor": "KY035", "command": "read_field", "interval": interval, "duration": duration}
    response = sess.post("http://localhost:8000/send_cmd", json=cmd)
    id = None
    if response.status_code == 200:
        data = response.json()
...
async def read_switch(
    ...
    """
    Read switch state from the KY035 sensor.
    ...
    Returns:
    A switch state reading with time stamp, or list of readings if Real_time is True.
    """
    cmd = {"sensor": "KY035", "command": "read_switch", "interval": interval, "duration": duration}
    response = sess.post("http://localhost:8000/send_cmd", json=cmd)
    id = None
    if response.status_code == 200:
        data = response.json()
...
async def read_all(
    ...
    """
    Read all data (raw, voltage, field, switch) from the KY035 sensor.
    ...
    Returns:
    A full report with time stamp, or list of reports if Real_time is True.
    """
    cmd = {"sensor": "KY035", "command": "read_all", "interval": interval, "duration": duration}
    response = sess.post("http://localhost:8000/send_cmd", json=cmd)
    id = None
    if response.status_code == 200:
        data = response.json()
